...
namespace HtmlToXaml
{
    ...
    public class HtmlParser
    {
        ...
        private XmlElement ParseHtmlContent()
        {
            ...
            XmlElement htmlRootElement = _document.CreateElementNS(XhtmlNamespace,"html");
            ...
            while (_htmlLexicalAnalyzer.NextTokenType != HtmlTokenType.EOF)
            {
                if (_htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.OpeningTagStart)
                {
                    ...
                    if (_htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Name)
                    {
                        ...
                        // Create an element
                        XmlElement htmlElement = _document.CreateElementNS(XhtmlNamespace,htmlElementName);
        ...
        {
            XmlElement htmlElementCopy = _document.CreateElementNS( XhtmlNamespace,htmlElement.LocalName.ToString());
            for (int i = 0; i < htmlElement.Attributes.Count; i++)
            {
                
                var attribute = htmlElement.Attributes[i];
                htmlElementCopy.SetAttribute(attribute.NodeName, attribute.NodeValue.ToString());
            }
        ...
        private void OpenStructuringElement(XmlElement htmlElement)
        {
            ...
            if (HtmlSchema.IsBlockElement(htmlElement.LocalName.ToString()))
            {
                while (_openedElements.Count > 0 && HtmlSchema.IsInlineElement(_openedElements.Peek().LocalName.ToString()))
                {
            ...
            if (_openedElements.Count > 0)
            {
                ...
                if (HtmlSchema.ClosesOnNextElementStart(htmlParent.LocalName.ToString(), htmlElement.LocalName.ToString()))
                {
                    _openedElements.Pop();
        ...
        private bool IsElementOpened(string htmlElementName)
        {
            ...
            {
                if (openedElement.LocalName.ToString() == htmlElementName)
                {
                    return true;
                }
            }
        ...
        private void CloseElement(string htmlElementName)
        {
            ...
            if (_pendingInlineElements.Count > 0 && _pendingInlineElements.Peek().LocalName.ToString() == htmlElementName)
            {
            ...
            else if (IsElementOpened(htmlElementName))
            {
                while (_openedElements.Count > 1) // we never pop the last element - the artificial root
                {
                    ...
                    if (htmlOpenedElement.LocalName.ToString() == htmlElementName)
                    {
                        return;
                    }

                    if (HtmlSchema.IsInlineElement(htmlOpenedElement.LocalName.ToString()))
                    {
